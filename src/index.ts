...
const autolog = {
  log<T extends typeof svgIcons>(text = "", type: AutologLogType<T> | number | boolean = "", time: number | boolean = 2500, autoClose = true) {
  ...
  create<T extends Record<string, string>>(options: AutologOptions<T>) {
    ...
    return {
      log: this.log.bind(this) as <U extends AutologLogType<T>>(text?: string, type?: U | number | boolean, time?: number, autoClose?: boolean) => void,
    };
  }
};
// 定义颜色对象的类型
